...
2. **Conversation-scoped retrieval.** Every similarity query filters by
   `conversation_id`. Empty conversations can use a seeded demo knowledge base when
   `DEMO_KNOWLEDGE_CONVERSATION_ID` is configured; once a user uploads documents,
   retrieval stays scoped to that conversation.
...
5. **Confidence labels.** Derived from how many chunks and documents support the
...
### Why no authentication in the demo
Authentication, RBAC, and admin workflows are explicitly out of scope. Instead the
app uses **anonymous conversation scoping**: the browser stores a `conversation_id`
in local storage and all documents/messages are tied to it. The schema is designed so
`user_id` / `workspace_id` can be added later without restructuring.
...
```
Upload them in the UI to run the demo script in PRD §25.
Use **Strategy Deck** in the executive action bar to generate a sourced PPTX briefing
from the same approved corpus.

To let new empty conversations answer from this coherent ADGM demo corpus, start the
backend, then run:
```bash
cd backend
npm run seed:demo
```
Copy the printed `DEMO_KNOWLEDGE_CONVERSATION_ID=...` into `backend/.env` and
restart the backend. Uploaded documents still override the demo corpus for that
conversation.
...
| `DEMO_KNOWLEDGE_CONVERSATION_ID` | optional indexed demo corpus for empty conversations |
...
| Method | Path | Purpose |
|--------|------|---------|
| `POST` | `/api/conversations` | Create anonymous conversation |
...
- **No authentication.** Anonymous conversation scoping only; do not upload
  confidential documents (the UI shows this notice).
